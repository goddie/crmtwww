										...
										<tbody role="alert" aria-live="polite" aria-relevant="all">
											<c:forEach var="m" varStatus="status" items="${list}">
												<tr class="gradeA odd">
													...
													<td class="action" title=""><a style="display: none;">编辑
													</a> <c:if test="${m.status==0 }">
															<a class="btn btn-xs btn-primary"
																onclick="return confirm('确认通过?')"
																href="${pageContext.request.contextPath}/product/action/review?id=${m.id}&rs=1">
																通过 </a>
															<a class="btn btn-xs btn-primary"
																onclick="return confirm('确认拒绝?')"
